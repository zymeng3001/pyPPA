...
def create_statistics(self, graph_y_labels):
    if self.args.softmax_variant_attn in ['consmax', 'consmax_v2', 'polymax', 'strongermax']:
        ...
        for layer in range(self.args.n_layer):
            ...
            # Save input snapshot to CSV
            input_csv_filename = f"{self.iter_num}_layer_{layer}_input.csv"
            np.savetxt(os.path.join(self.args.out_dir, input_csv_filename), np.array(input_snapshot), delimiter=",")
            ...
            # Save output snapshot to CSV
            output_csv_filename = f"{self.iter_num}_layer_{layer}_output.csv"
            np.savetxt(os.path.join(self.args.out_dir, output_csv_filename), np.array(output_snapshot), delimiter=",")

            # BETA and GAMMA (if applicable)
            if self.args.softmax_variant_attn == 'consmax':
                ...
                gammas.append(gamma[0].item())
                betas.append(beta[0].item())
        ...
        # Save CSV outputs for inputs and outputs (summarized)
