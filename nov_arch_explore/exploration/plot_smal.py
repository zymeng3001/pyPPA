...
for metric, label in metrics.items():
    # Select top 10% by the current metric (>= 90th percentile)
    thr = df[metric].quantile(0.9)
    df_plot = df[df[metric] >= thr].copy()
    print(f"{metric}: plotting top 10% (n={len(df_plot)}) of total {len(df)}; threshold={thr:.4g}")
