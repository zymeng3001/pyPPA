...
problem.search_space.root.add_discrete_param(name='wmem_depth', feasible_values=[32, 64, 128] + np.arange(256, 4352, 256).tolist(), default_value=256)
problem.search_space.root.add_discrete_param(name='cache_depth', feasible_values=[32, 64, 128] + np.arange(256, 4352, 256).tolist(), default_value=256)
...
study_client = clients.Study.from_study_config(
  study_config,
  owner='ppa_runner',
  study_id='ppa_core_array_top_4_6'
)
...
def vizier_optimizer(prev_iter_number, prev_iter_ppa_runs: list[PPARunner], previous_suggestions):
    ...
    if prev_iter_number >= 1000: # Run for 10 iterations and then stop
        print("Optimization complete.")

        with open('seen_configs_core_top.pkl', 'wb') as f:
            pickle.dump(seen_configs, f)
        ...
        'next_suggestions': [
            {
                ...
                'hyperparameters': {
                    ...
                    'wmem_depth': int(suggestion.parameters['wmem_depth']),
                    'cache_depth': int(suggestion.parameters['cache_depth'])
                }
            } for suggestion in feasible_suggestions
        ],
